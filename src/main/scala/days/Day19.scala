...
object Day19 extends AdventOfCode {
  ...
  private def solvePart1(rating: Map[String, Int], currentWorkFlowName: String, workFlowMap: Map[String, WorkFlow]): Boolean = {
    currentWorkFlowName match {
      case "A" => true
      case "R" => false
      case _ =>
        workFlowMap(currentWorkFlowName)
          .boundaries
  ...
  private def parseWorkflow(lines: List[String]): List[WorkFlow] = {
    lines.takeWhile(_.nonEmpty).map { line =>
      ...
      val boundaries = (1 until workflowString.size - 1)
        .flatMap(i => boundaryRegex.findAllMatchIn(workflowString(i))
