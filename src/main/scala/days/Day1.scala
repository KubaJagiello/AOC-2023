...
object Day1 extends AdventOfCode {
  ...
  def part1(lines: List[String]): String = lines.foldLeft(0) {
    (acc, line) => acc + (line.find(_.isDigit).get.toString + line.findLast(_.isDigit).get.toString).toInt
  }.toString

  def part2(lines: List[String]): String = {
    val map = HashMap[String, Int]("one" -> 1, "two" -> 2, "three" -> 3, "four" -> 4, "five" -> 5, "six" -> 6, "seven" -> 7, "eight" -> 8, "nine" -> 9)
    ...
    lines.foldLeft(0) { (acc, line) =>
      ...
      map.keys.foreach {
        sub =>
          Iterator.iterate(line.indexOf(sub)) { idx => if (idx != -1) line.indexOf(sub, idx + sub.length) else -1 }
            .takeWhile(_ != -1)
